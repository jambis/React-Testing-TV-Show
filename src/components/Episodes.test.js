...
test("Component renders with empty array at first and rerenders with show episodes as the new episodes prop value", () => {
  // initial render with empty list of episodes
  const { rerender, queryByTestId, getAllByText } = render(
    <Episodes episodes={[]} />
  );
  ...
  expect(queryByTestId("episodes")).toHaveTextContent(
    "Chapter One: The Vanishing of Will Byers"
  );
